...
import fr.labri.popsycle.io.CDGWriter;
import fr.labri.popsycle.model.JClassGroup;
import fr.labri.popsycle.model.JPackageGroup;

public class DepsViewUI extends Composite implements SelectionListener {

	private JPackageDepsTree tree;
	...
	public static void main (String [] args) throws Exception {
		...
		shell.setText("DepsView");
		shell.setLayout(new FillLayout());
		new DepsViewUI(shell,SWT.NONE);
	...
	public DepsViewUI(Composite parent,int style) {
		super(parent,style);

		this.setLayout(new GridLayout(1,true));

		tree = new JPackageDepsTree(this, SWT.BORDER);
	...
	private void initMenu() {
		...
		infoAboutItem.setText("&About DepsView");
		infoAboutItem.addSelectionListener(this);
	}

	public void setJavaResource(String[] res) throws IOException, ClassFormatException, ClassNotFoundException {
	...
	public void setCdgFile(String file) throws ClassFormatException, IOException, ClassNotFoundException {
	...
	private void updateStatusBar() {
		status.setText(pkgGrp.toString());	
	}
	...
	private void handleMetricsSelected() {
		if ( pkgGrp != null ) {
			...
			box.setMessage(pkgGrp.getJPackages().size() + " packages.\n" + 
					pkgGrp.getJPackageDepsNb() + " package dependencies (" + tree.getNotSureJPackageDepNb() + " untagged).\n" + 
	...
	private void handleWriteResultsSelected() {
		if ( !tree.isEmpty() ) {
			...
			try {
				if ( !"".equals(resource) && !(resource == null) ) {
					this.tree.writeResultsInFile(resource + ".deps");
					CDGWriter writer = new CDGWriter(clsGrp);
					writer.write(new File(resource + ".cdg"));
				}
			}
			catch( Exception e ) {
				showException(e);
	...
	private void handleAboutSelected() {
		MessageBox box = new MessageBox(this.getShell());
		box.setText("About DepsView");
		box.setMessage("DepsView, a Java package dependencies analyzer\n\n" +
