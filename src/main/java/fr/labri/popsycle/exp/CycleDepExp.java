...
public class CycleDepExp {
	public static void main(String[] args) throws Exception {
		JClassGroupReader imp = new CDGReader("cdg/geco_3-3-2.cdg");
		
		JClassGroup clsGrp = imp.read()	;
		JPackageGroup pkgGrp = clsGrp.extractJPackageGroup();

		JPackageDepsFinder f = new CycleDepsFinder(pkgGrp);
		Set<JPackageDep> cd = f.extractJPackageDeps();
		
		JPackageDepsExporter.exportToFile(cd,"tmp/pharo_cd.csv");
	}
}
