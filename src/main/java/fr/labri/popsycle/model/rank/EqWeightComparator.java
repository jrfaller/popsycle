...
public class EqWeightComparator implements Comparator<JPackageCycle>{
	
	private double maxWeight = Double.MIN_VALUE;
	
	public EqWeightComparator(Collection<JPackageCycle> cycles) {
		for( JPackageCycle cycle : cycles ) {
			double w = WeightUtils.getEqWeight(cycle);
			if ( w > maxWeight )
				maxWeight = w;
		}
	}
	...
	public double getWeight(JPackageCycle cycle) {
		return WeightUtils.getEqWeight(cycle) / maxWeight;
	}
